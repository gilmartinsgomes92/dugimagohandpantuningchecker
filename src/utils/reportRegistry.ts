...
export function createCertificationReportRecord(params: {
  ...
  const orderedAggregates = orderCertificationAggregates(aggregates);

  return {
    verificationId,
    createdAt,
    noteCount: orderedAggregates.length,
    aggregates: orderedAggregates.map(toSerializableAggregate),
    stats,
    verdict,
