 ...
 * Handles note naming, frequency calculations, cents deviation,
 * and robust averaging helpers used by the quick tuning flow.
 */
...
/**
 * Robust mean for partial-frequency aggregation.
 *
 * Rules:
 * - 0 values  -> null
 * - 1 value   -> that value
 * - 2 values  -> arithmetic mean
 * - 3 values  -> median
 * - 4+ values -> trimmed mean (remove the lowest and highest values first)
 */
export function robustPartialMean(values: number[]): number | null {
