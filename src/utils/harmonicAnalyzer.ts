...
const DEFAULT_SEARCH_CENTS = 110; // Search window in cents around expected harmonic
...
export function findHarmonicFrequency(
  ...
  sampleRate: number,
  fftSize: number,
  searchCents: number = DEFAULT_SEARCH_CENTS,
): number | null {
  const binHz = sampleRate / fftSize;
  const numBins = freqData.length; // = fftSize / 2

  // Calculate search range in bins
  const lowFreq = targetFreq / centsToFrequencyRatio(searchCents);
  const highFreq = targetFreq * centsToFrequencyRatio(searchCents);
  ...
  // Guard against leakage from a stronger peak that lives outside the requested window.
  // When that happens, the loudest bin inside the window tends to sit exactly on the
  // window edge and the slope keeps rising toward the real peak outside the window.
  const atLowEdge = peakBin === lowBin;
  const atHighEdge = peakBin === highBin;

  if (atLowEdge && freqData[peakBin] <= freqData[peakBin + 1]) return null;
  if (atHighEdge && freqData[peakBin] <= freqData[peakBin - 1]) return null;
