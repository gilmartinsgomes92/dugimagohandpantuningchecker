...
export function detectPitchInWindow(
  ...
  // search window. If the "peak" sits on or very near a window edge and the
  // spectrum is still sloping toward the outside of the window, reject it.
  const lowShoulder = Math.max(1, lowBin - 2);
  const highShoulder = Math.min(numBins - 2, highBin + 2);
  const nearLowEdge = peakBin <= lowBin + 1;
  const nearHighEdge = peakBin >= highBin - 1;

  if (nearLowEdge) {
    const outsideLowMag = Math.max(magDB[lowShoulder], magDB[Math.max(1, lowBin - 1)]);
    if (outsideLowMag >= peakMag - 1.5 || magDB[peakBin] <= magDB[Math.min(numBins - 2, peakBin + 1)]) {
      return null;
    }
  }

  if (nearHighEdge) {
    const outsideHighMag = Math.max(magDB[highShoulder], magDB[Math.min(numBins - 2, highBin + 1)]);
    if (outsideHighMag >= peakMag - 1.5 || magDB[peakBin] <= magDB[Math.max(1, peakBin - 1)]) {
      return null;
    }
  }
