...
export function detectPitchInWindow(
  ...
  if (atLowEdge) {
    if (magDB[peakBin] <= magDB[peakBin + 1]) return null;
    if (peakBin > 1 && magDB[peakBin - 1] >= magDB[peakBin] - 1.5) return null;
  }
  if (atHighEdge) {
    if (magDB[peakBin] <= magDB[peakBin - 1]) return null;
    if (peakBin + 1 < numBins && magDB[peakBin + 1] >= magDB[peakBin] - 1.5) return null;
  }
