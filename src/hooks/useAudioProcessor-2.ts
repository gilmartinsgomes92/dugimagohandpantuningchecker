...
const PARTIAL_HISTORY_SIZE = 8;
const PARTIAL_MIN_FRAMES = 3;
const PARTIAL_MAX_MAD_CENTS = 12;
const PARTIAL_MAX_ABS_CENTS = 150;
const PARTIAL_MISS_RESET_FRAMES = 5;

type WindowFrame = { freq: number; cents: number; quality: number };
type PartialFrame = { freq: number; cents: number };
...
function updatePartialLock(
  framesRef: { current: PartialFrame[] },
  missCountRef: { current: number },
  candidateFreq: number | null,
  targetFreq: number,
): number | null {
  if (candidateFreq === null) {
    missCountRef.current += 1;
    if (missCountRef.current >= PARTIAL_MISS_RESET_FRAMES) {
      framesRef.current = [];
    }
  } else {
    missCountRef.current = 0;
    const cents = centsFromNominal(candidateFreq, targetFreq);
    if (Math.abs(cents) <= PARTIAL_MAX_ABS_CENTS) {
      const history = framesRef.current;
      if (history.length >= 2) {
        const historyMed = median(history.map((f) => f.cents));
        if (Math.abs(cents - historyMed) <= 35) {
          history.push({ freq: candidateFreq, cents });
        }
      } else {
        history.push({ freq: candidateFreq, cents });
      }
      if (history.length > PARTIAL_HISTORY_SIZE) history.shift();
    }
  }

  if (framesRef.current.length < PARTIAL_MIN_FRAMES) return null;

  const centsArr = framesRef.current.map((f) => f.cents);
  const centsMed = median(centsArr);
  const centsMad = mad(centsArr, centsMed);
  if (centsMad > PARTIAL_MAX_MAD_CENTS) return null;

  const freqMed = median(framesRef.current.map((f) => f.freq));
  return freqMed;
}
...
export const useAudioProcessor = () => {
  ...
  const octaveFramesRef = useRef<PartialFrame[]>([]);
  const cfifthFramesRef = useRef<PartialFrame[]>([]);
  const octaveMissCountRef = useRef<number>(0);
  const cfifthMissCountRef = useRef<number>(0);
  ...
  const resetState = useCallback(() => {
    ...
    octaveMissCountRef.current = 0;
    cfifthMissCountRef.current = 0;
...
if (rms >= dynamicGate) {
          ...
          if (match !== null) {
            ...
            if (freq !== null) {
              ...
              const lockedOctave = updatePartialLock(
                octaveFramesRef,
                octaveMissCountRef,
                octaveFreq,
                octaveNominal,
              );
              const lockedCFifth = updatePartialLock(
                cfifthFramesRef,
                cfifthMissCountRef,
                compFifthCandidate,
                compFifthNominal,
              ...
              smoothedOctaveRef.current =
                lockedOctave === null
                  ? octaveMissCountRef.current >= PARTIAL_MISS_RESET_FRAMES
                    ? null
                    : smoothedOctaveRef.current
                  : smoothedOctaveRef.current === null
                    ? lockedOctave
                    : FREQ_SMOOTH_ALPHA * lockedOctave + (1 - FREQ_SMOOTH_ALPHA) * smoothedOctaveRef.current;

              smoothedCFifthRef.current =
                lockedCFifth === null
                  ? cfifthMissCountRef.current >= PARTIAL_MISS_RESET_FRAMES
                    ? null
                    : smoothedCFifthRef.current
                  : smoothedCFifthRef.current === null
                    ? lockedCFifth
                    : FREQ_SMOOTH_ALPHA * lockedCFifth + (1 - FREQ_SMOOTH_ALPHA) * smoothedCFifthRef.current;
