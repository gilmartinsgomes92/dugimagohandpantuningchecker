...
/**
 * Extreme-detune fallback is only used after repeated misses while the note lock
 * is still good. This keeps the near-zero path precise, but lets the app find
 * genuine octave / compound-fifth detunes in the +300c / +400c range.
 */
const EXTREME_PARTIAL_TRIGGER_MISSES = 3;
const OCTAVE_EXTREME_ACQUIRE_WINDOW_CENTS = 680;
const CFIFTH_EXTREME_ACQUIRE_WINDOW_CENTS = 680;

/**
 * Broad sanity bounds for real handpan partials.
 * These are intentionally wider than the old "normal" harmonic limits so true
 * extreme deviations are not auto-rejected to blank.
 */
...
function partialWindowCents(
  ...
  hasHistory: boolean,
  useExtremeAcquire: boolean,
): number {
  if (harmonicType === 'octave') {
    if (hasHistory) return OCTAVE_TRACK_WINDOW_CENTS;
    return useExtremeAcquire
      ? OCTAVE_EXTREME_ACQUIRE_WINDOW_CENTS
      : OCTAVE_ACQUIRE_WINDOW_CENTS;
  }
  if (hasHistory) return CFIFTH_TRACK_WINDOW_CENTS;
  return useExtremeAcquire
    ? CFIFTH_EXTREME_ACQUIRE_WINDOW_CENTS
    : CFIFTH_ACQUIRE_WINDOW_CENTS;
}
...
function getRatioQuality(
  ...
  // Soft prior only: keep rewarding close-to-ideal partials, but do not punish
  // real extreme deviations so hard that they disappear to blank.
                ...
                if (refinedOctave !== null) {
                  ...
                  pushFrame(
                    octaveFramesRef.current,
                    {
                      ...
                      quality: clamp(
                        0.42 * score + 0.28 * octaveRatioQuality + 0.3 * octaveAgreement,
                        0,
                        1,
                      ),
                      ts: nowMs,
                ...
                if (refinedCFifth !== null) {
                  ...
                  pushFrame(
                    cFifthFramesRef.current,
                    {
                      ...
                      quality: clamp(
                        0.42 * score + 0.28 * cFifthRatioQuality + 0.3 * cFifthAgreement,
                        0,
                        1,
                      ),
                      ts: nowMs,
