...
const OCTAVE_ACQUIRE_WINDOW_CENTS = 700;
const OCTAVE_EXTREME_ACQUIRE_WINDOW_CENTS = 800;
const OCTAVE_TRACK_WINDOW_CENTS = 90;
const CFIFTH_ACQUIRE_WINDOW_CENTS = 700;
const CFIFTH_EXTREME_ACQUIRE_WINDOW_CENTS = 800;
...
function finalizeProvisionalFrequency(
  ...
  nowMs: number,
  stableWindowMs: number,
  minQuality: number = 0.46,
  madFactor: number = 0.55,
): number | null {
  ...
  if (centsMad > Math.max(10, maxMadCents * madFactor)) return null;
  if (qualityMed < minQuality) return null;
