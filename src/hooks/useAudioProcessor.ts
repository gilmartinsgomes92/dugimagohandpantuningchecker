...
const SILENCE_GRACE_FRAMES = 180;
...
const PARTIAL_STABLE_WINDOW_MS = 2200;
const PARTIAL_HOLD_MS = 4200;
...
const EXTREME_PARTIAL_CENTS_THRESHOLD = 140;
...
function finalizeStableFrequency(
  ...
  const isExtreme = Math.abs(centsMed) >= EXTREME_PARTIAL_CENTS_THRESHOLD;
  if (centsMad > (isExtreme ? Math.max(maxMadCents * 1.45, 30) : maxMadCents)) return null;

  const selected = recentFrames
    .filter((f) =>
      Math.abs(f.cents - centsMed) <= Math.max(maxMadCents * (isExtreme ? 2.2 : 1.8), isExtreme ? 34 : 18)
    )
    .sort((a, b) => (b.quality - a.quality) || (b.ts - a.ts))
    .slice(0, Math.max(minFrames, Math.ceil(recentFrames.length * (isExtreme ? 0.6 : 0.75))))
    .map((f) => f.freq);
...
function finalizeProvisionalFrequency(
  ...
  const recentFrames = pruneFrames(frames, nowMs, Math.min(stableWindowMs, 1100)).slice(-needed);
  ...
  if (centsMad > Math.max(isExtreme ? 22 : 10, maxMadCents * (isExtreme ? 0.95 : 0.55))) return null;
  if (qualityMed < (isExtreme ? 0.33 : 0.46)) return null;
