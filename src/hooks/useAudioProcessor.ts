...
function partialWindowCents(
  ...
  if (harmonicType === 'octave') {
    return hasHistory ? OCTAVE_TRACK_WINDOW_CENTS : OCTAVE_ACQUIRE_WINDOW_CENTS;
  }
  return hasHistory ? CFIFTH_TRACK_WINDOW_CENTS : CFIFTH_ACQUIRE_WINDOW_CENTS;
}
...
function getRatioQuality(
  ...
  return clamp(1 - ratioCents / 180, 0, 1);
}
          ...
          if (match !== null) {
            ...
            if (freq !== null) {
              ...
              if (shouldTrackPartials) {
                const octaveSearchCenter = smoothedOctaveRef.current ?? smoothedFreq * 2;
                ...
                const cFifthSearchCenter = smoothedCFifthRef.current ?? smoothedFreq * 3;
