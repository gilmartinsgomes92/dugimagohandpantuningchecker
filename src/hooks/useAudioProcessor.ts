...
const EXTREME_MIN_OCTAVE_FRAMES = 6;
const MAX_OCTAVE_FRAMES = 56;
const MIN_CFIFTH_FRAMES = 4;
const EXTREME_MIN_CFIFTH_FRAMES = 6;
const MAX_CFIFTH_FRAMES = 56;

const OCTAVE_MAX_MAD_CENTS = 22;
const OCTAVE_EXTREME_MAX_MAD_CENTS = 28;
const CFIFTH_MAX_MAD_CENTS = 26;
const CFIFTH_EXTREME_MAX_MAD_CENTS = 32;

const OCTAVE_NORMAL_ACQUIRE_WINDOW_CENTS = 70;
const OCTAVE_NORMAL_TRACK_WINDOW_CENTS = 45;
const CFIFTH_NORMAL_ACQUIRE_WINDOW_CENTS = 90;
const CFIFTH_NORMAL_TRACK_WINDOW_CENTS = 55;

const EXTREME_SCAN_RANGE_CENTS = 700;
const EXTREME_SCAN_STEP_CENTS = 35;
const EXTREME_SCAN_PROBE_CENTS = 28;
const EXTREME_MIN_CLUSTER_VOTES = 3;
const EXTREME_TRIGGER_MISSES = 3;

const OCTAVE_MIN_RATIO = 1.35;
const OCTAVE_MAX_RATIO = 2.90;
const CFIFTH_MIN_RATIO = 1.95;
...
function finalizeStableFrequency(
  frames: WindowFrame[],
  minFrames: number,
  extremeMinFrames: number,
  maxMadCents: number,
  extremeMaxMadCents: number,
  nowMs: number,
  ...
  const isExtreme = Math.abs(centsMed) >= 150;
  const requiredMinFrames = isExtreme ? extremeMinFrames : minFrames;
  const allowedMad = isExtreme ? extremeMaxMadCents : maxMadCents;

  if (recentFrames.length < requiredMinFrames) return null;

  const centsMad = mad(centsArr, centsMed);
  if (centsMad > allowedMad) return null;

  const selected = recentFrames
    .filter((f) => Math.abs(f.cents - centsMed) <= Math.max(allowedMad * 1.8, 18))
    .sort((a, b) => (b.quality - a.quality) || (b.ts - a.ts))
    .slice(0, Math.max(requiredMinFrames, Math.ceil(recentFrames.length * 0.75)))
    .map((f) => f.freq);

  if (selected.length < requiredMinFrames) return null;
  return median(selected);
}

type PartialScanResult = { freq: number; votes: number; spreadCents: number };
...
function detectPartialCandidate(
  ...
  return isRatioSane(candidate, lockedFundamental, harmonicType) ? candidate : null;
}

function isRatioSane(
  candidate: number,
  lockedFundamental: number,
  harmonicType: 'octave' | 'compoundFifth',
): boolean {
...
function freqAtCentsOffset(targetFreq: number, cents: number): number {
  return targetFreq * Math.pow(2, cents / 1200);
}

function scanExtremePartialCandidate(
  ...
  lockedFundamental: number,
  harmonicType: 'octave' | 'compoundFifth',
): PartialScanResult | null {
  const nyquist = sampleRate / 2;
  const rawCandidates: number[] = [];

  for (
    let cents = -EXTREME_SCAN_RANGE_CENTS;
    cents <= EXTREME_SCAN_RANGE_CENTS;
    cents += EXTREME_SCAN_STEP_CENTS
  ) {
    const center = freqAtCentsOffset(targetFreq, cents);
    if (center <= 0 || center >= nyquist - 5) continue;

    const candidate = findHarmonicFrequency(
      freqData,
      center,
      sampleRate,
      fftSize,
      EXTREME_SCAN_PROBE_CENTS,
    );

    if (candidate === null) continue;
    if (!isRatioSane(candidate, lockedFundamental, harmonicType)) continue;

    rawCandidates.push(candidate);
  }

  if (rawCandidates.length < EXTREME_MIN_CLUSTER_VOTES) return null;
  ...
  for (const candidate of sorted) {
    ...
    const clusterMedian = median(lastCluster);
    const deltaCents = Math.abs(centsFromNominal(candidate, clusterMedian));
    if (deltaCents <= 18) {
  ...
  const ranked = clusters
    .map((cluster) => {
      ...
      const offsets = cluster.map((freq) => centsFromNominal(freq, clusterMedian));
      const spreadCents = offsets.length > 1 ? Math.max(...offsets) - Math.min(...offsets) : 0;
      return {
        freq: clusterMedian,
        votes: cluster.length,
        spreadCents,
        targetOffsetCents: Math.abs(centsFromNominal(clusterMedian, targetFreq)),
      };
    })
    .filter((cluster) => cluster.votes >= EXTREME_MIN_CLUSTER_VOTES && cluster.spreadCents <= 26)
    .sort(
      (a, b) =>
        (b.votes - a.votes) ||
        (a.spreadCents - b.spreadCents) ||
        (a.targetOffsetCents - b.targetOffsetCents),
    );
  ...
  return {
    ...
    spreadCents: ranked[0].spreadCents,
  };
}
          ...
          if (match !== null) {
            ...
            if (freq !== null) {
              ...
              if (shouldTrackPartials) {
                ...
                const octaveSearchCents = partialWindowCents('octave', smoothedOctaveRef.current !== null);
                ...
                const octaveFft =
                  octaveFftRaw !== null && isRatioSane(octaveFftRaw, smoothedFreq, 'octave')
                    ? octaveFftRaw
                    : null;
                ...
                const refinedOctave = chooseBestPartialCandidate(
                  octaveFft,
                  octaveTime,
                  smoothedOctaveRef.current ?? octaveNominal,
                );
                ...
                if (refinedOctave !== null) {
                  pushFrame(
                    octaveFramesRef.current,
                    {
                      ...
                      quality: clamp(0.6 * score + 0.4 * octaveAgreement, 0, 1),
                      ts: nowMs,
                    },
                    MAX_OCTAVE_FRAMES,
                  );
                  ...
                  const octaveExtreme =
                    octaveMissesRef.current >= EXTREME_TRIGGER_MISSES
                      ? scanExtremePartialCandidate(
                          freqBufRef.current,
                          sampleRate,
                          analyserRef.current.fftSize,
                          octaveNominal,
                  ...
                  if (octaveExtreme !== null) {
                    const consensus = clamp((octaveExtreme.votes - 2) / 4, 0, 1);
                    const tightness = clamp(1 - octaveExtreme.spreadCents / 24, 0, 1);
                    pushFrame(
                      octaveFramesRef.current,
                      {
                        freq: octaveExtreme.freq,
                        cents: centsFromNominal(octaveExtreme.freq, octaveNominal),
                        quality: clamp(0.25 * score + 0.45 * consensus + 0.3 * tightness, 0, 1),
                        ts: nowMs,
                      },
                      MAX_OCTAVE_FRAMES,
                    );
                ...
                const cFifthFft =
                  cFifthFftRaw !== null && isRatioSane(cFifthFftRaw, smoothedFreq, 'compoundFifth')
                    ? cFifthFftRaw
                    : null;
                const cFifthTime =
                ...
                const refinedCFifth = chooseBestPartialCandidate(
                  cFifthFft,
                  cFifthTime,
                  smoothedCFifthRef.current ?? compFifthNominal,
                );
                ...
                if (refinedCFifth !== null) {
                  pushFrame(
                    cFifthFramesRef.current,
                    {
                      ...
                      quality: clamp(0.6 * score + 0.4 * cFifthAgreement, 0, 1),
                      ts: nowMs,
                    },
                    MAX_CFIFTH_FRAMES,
                  );
                  ...
                  const cFifthExtreme =
                    cFifthMissesRef.current >= EXTREME_TRIGGER_MISSES
                      ? scanExtremePartialCandidate(
                          freqBufRef.current,
                          sampleRate,
                          analyserRef.current.fftSize,
                  ...
                  if (cFifthExtreme !== null) {
                    const consensus = clamp((cFifthExtreme.votes - 2) / 4, 0, 1);
                    const tightness = clamp(1 - cFifthExtreme.spreadCents / 24, 0, 1);
                    pushFrame(
                      cFifthFramesRef.current,
                      {
                        freq: cFifthExtreme.freq,
                        cents: centsFromNominal(cFifthExtreme.freq, compFifthNominal),
                        quality: clamp(0.25 * score + 0.45 * consensus + 0.3 * tightness, 0, 1),
                        ts: nowMs,
                      },
                      MAX_CFIFTH_FRAMES,
              ...
              const stableOctave = canDisplayPartials
                ? finalizeStableFrequency(
                    octaveFramesRef.current,
                    MIN_OCTAVE_FRAMES,
                    EXTREME_MIN_OCTAVE_FRAMES,
                    OCTAVE_MAX_MAD_CENTS,
                    OCTAVE_EXTREME_MAX_MAD_CENTS,
                    nowMs,
              ...
              const stableCFifth = canDisplayPartials
                ? finalizeStableFrequency(
                    cFifthFramesRef.current,
                    MIN_CFIFTH_FRAMES,
                    EXTREME_MIN_CFIFTH_FRAMES,
                    CFIFTH_MAX_MAD_CENTS,
                    CFIFTH_EXTREME_MAX_MAD_CENTS,
                    nowMs,
                    PARTIAL_STABLE_WINDOW_MS,
                  )
                : null;
