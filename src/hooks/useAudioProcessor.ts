...
const PARTIAL_DISPLAY_DELAY_MS = 260;
const PARTIAL_TRACK_WINDOW_MS = 4200;
const PARTIAL_STABLE_WINDOW_MS = 1800;
const PARTIAL_HOLD_MS = 2600;

const SUSTAIN_GATE_MULTIPLIER = 0.42;
const SUSTAIN_NOISE_MULTIPLIER = 3.2;
const SUSTAIN_GATE_FLOOR = 0.0022;
...
const OCTAVE_MAX_MAD_CENTS = 28;
const CFIFTH_MAX_MAD_CENTS = 34;

const OCTAVE_ACQUIRE_WINDOW_CENTS = 140;
const OCTAVE_EXTREME_WINDOW_CENTS = 720;
const OCTAVE_TRACK_WINDOW_CENTS = 70;
const CFIFTH_ACQUIRE_WINDOW_CENTS = 170;
const CFIFTH_EXTREME_WINDOW_CENTS = 760;
const CFIFTH_TRACK_WINDOW_CENTS = 90;

const OCTAVE_MIN_RATIO = 1.42;
...
function partialWindowCents(
  ...
  if (harmonicType === 'octave') {
    ...
    return misses >= 2 ? OCTAVE_EXTREME_WINDOW_CENTS : OCTAVE_ACQUIRE_WINDOW_CENTS;
  }
  if (hasHistory) return CFIFTH_TRACK_WINDOW_CENTS;
  return misses >= 2 ? CFIFTH_EXTREME_WINDOW_CENTS : CFIFTH_ACQUIRE_WINDOW_CENTS;
}
...
function getRatioQuality(
  ...
  return clamp(1 - ratioCents / 520, 0, 1);
}
...
function hasActiveLockedNote(
  ...
  return (
    ...
    smoothedFreq !== null &&
    strikeAtMs > 0 &&
    nowMs - strikeAtMs <= PARTIAL_TRACK_WINDOW_MS
  );
}
...
const activeLockedNote = hasActiveLockedNote(
  smoothedMidiRef.current,
  smoothedFreqRef.current,
  strikeAtMsRef.current,
  nowMs,
);
const sustainGate = activeLockedNote
  ? Math.max(
      SIGNAL_RMS_THRESHOLD * SUSTAIN_GATE_MULTIPLIER,
      noiseFloorRef.current * SUSTAIN_NOISE_MULTIPLIER + 0.00025,
      SUSTAIN_GATE_FLOOR,
    )
  : dynamicGate;

if (rms >= dynamicGate || (activeLockedNote && rms >= sustainGate)) {
          ...
          if (match !== null || activeLockedNote) {
            silenceCountRef.current = 0;
            let midiNote: number;
            let nominalFreq: number;
            let score: number;
            const sampleRate = audioCtxRef.current.sampleRate;

            if (match !== null) {
              ({ midiNote, nominalFreq, score } = match);
              ...
              score = Math.max(lastLockQualityRef.current * 0.92, 0.5);
              lastMatchScoreRef.current = score;
              lastNoteNameRef.current = midiToFullName(midiNote);
            }
            ...
            if (freq !== null) {
              ...
              if (shouldTrackPartials) {
                ...
                const octaveSearchCents = partialWindowCents(
                  'octave',
                  smoothedOctaveRef.current !== null,
                  octaveMissesRef.current,
                );
                ...
                const octaveTime = detectPartialCandidate(
                  buf,
                  sampleRate,
                  octaveFft ?? octaveSearchCenter,
                  smoothedFreq,
                  'octave',
                  octaveFft !== null ? Math.max(48, Math.min(76, Math.round(octaveSearchCents * 0.24))) : octaveSearchCents,
                );
                ...
                if (refinedOctave !== null) {
                  ...
                  pushFrame(
                    octaveFramesRef.current,
                    {
                      ...
                      quality: clamp(0.45 * score + 0.35 * octaveRatioQuality + 0.2 * octaveAgreement, 0, 1),
                      ts: nowMs,
                    },
                    MAX_OCTAVE_FRAMES,
                  );
                ...
                const cFifthFft =
                  cFifthFftRaw !== null && isRatioValid(cFifthFftRaw, smoothedFreq, 'compoundFifth')
                    ...
                    : null;
                const cFifthTime =
                  (cFifthFft ?? cFifthSearchCenter) <= sampleRate / 2
                    ? detectPartialCandidate(
                        buf,
                        sampleRate,
                        cFifthFft ?? cFifthSearchCenter,
                        smoothedFreq,
                        'compoundFifth',
                        cFifthFft !== null ? Math.max(54, Math.min(90, Math.round(cFifthSearchCents * 0.24))) : cFifthSearchCents,
                      )
                    : null;
                ...
                if (refinedCFifth !== null) {
                  ...
                  pushFrame(
                    cFifthFramesRef.current,
                    {
                      ...
                      quality: clamp(0.45 * score + 0.35 * cFifthRatioQuality + 0.2 * cFifthAgreement, 0, 1),
                      ts: nowMs,
                    },
                    MAX_CFIFTH_FRAMES,
                  );
            ...
            rejectReasonRef.current = activeLockedNote ? 'holding locked note' : 'no match';
            if (activeLockedNote) {
          ...
          silenceCountRef.current += 1;
          if (activeLockedNote && silenceCountRef.current < SILENCE_GRACE_FRAMES) {
            setResult((prev) => ({
              ...prev,
              matchScore: Math.max(prev.matchScore, lastMatchScoreRef.current * 0.88),
              lockQuality: Math.max(prev.lockQuality, lastLockQualityRef.current * 0.9),
            }));
          }
          if (silenceCountRef.current >= SILENCE_GRACE_FRAMES) {
            resetState();
