...
interface AudioResult {
  ...
  lockQuality: number;
  // 0–1 progress of the longer partial refinement phase.
  refinementProgress: number;
  // True once the note has had enough time for trustworthy partial capture.
...
const LOCK_WINDOW_MS = 420; // longer fundamental lock for low handpan notes
const REFINE_WINDOW_MS = 1600; // longer sustain tracking for octave / compound fifth
const MIN_LOCK_FRAMES = 4;
const MAX_LOCK_FRAMES = 22;
const MIN_PARTIAL_FRAMES = 3;
const MAX_PARTIAL_FRAMES = 24;
const PARTIAL_CLEAR_MISSES = 8;
const REFINEMENT_READY_MS = 320;

// Hold a recently confirmed note through natural sustain, even if matchNote briefly drops.
const HOLD_LAST_NOTE_MS = 1400;
const SUSTAIN_RMS_FACTOR = 2.8;
const SUSTAIN_MIN_LOCK_QUALITY = 0.48;

const WIDE_OCTAVE_WINDOW_CENTS = 180;
const WIDE_CFIFTH_WINDOW_CENTS = 380;
...
export const useAudioProcessor = () => {
  ...
  // Last trusted detection, used to carry a good note through sustain
  const lastGoodDetectionMsRef = useRef<number>(0);
  const lastGoodNominalFreqRef = useRef<number | null>(null);
  const lastGoodMidiRef = useRef<number | null>(null);
  const lastGoodMatchScoreRef = useRef<number>(0);
  ...
  const resetState = useCallback(() => {
    ...
    lastGoodDetectionMsRef.current = 0;
    lastGoodNominalFreqRef.current = null;
    lastGoodMidiRef.current = null;
    lastGoodMatchScoreRef.current = 0;
      ...
      const tick = () => {
        ...
        if (rms >= dynamicGate) {
          ...
          if (match !== null) {
            ...
            // --- 2-phase note read: fast fundamental lock + longer partial refinement ---
            ...
            const lockStartMs = IGNORE_AFTER_STRIKE_MS;
            const lockEndMs = IGNORE_AFTER_STRIKE_MS + LOCK_WINDOW_MS;
            const refineEndMs = lockEndMs + REFINE_WINDOW_MS;

            if (dt >= lockStartMs && dt <= lockEndMs && freq !== null) {
              const cents = clamp(centsFromNominal(freq, nominalFreq), -60, 60);
              pushFrame(windowFramesRef.current, { freq, cents, quality: score }, MAX_LOCK_FRAMES);
            }

            let lockedFreq: number | null = null;
            if (windowFramesRef.current.length >= MIN_LOCK_FRAMES) {
              ...
              if (lockQuality >= 0.55) lockedFreq = freqMed;
            } else {
            ...
            if (lockedFreq === null && smoothedFreqRef.current !== null) {
              lockedFreq = smoothedFreqRef.current;
            }
            ...
            lastGoodDetectionMsRef.current = nowMs;
            lastGoodNominalFreqRef.current = nominalFreq;
            lastGoodMidiRef.current = midiNote;
            lastGoodMatchScoreRef.current = score;
            ...
            if (dt >= lockEndMs && dt <= refineEndMs) {
              const octaveNominal = nominalFreq * 2;
              ...
              let refinedCFifth: number | null = null;
              if (compFifthNominal <= sampleRate / 2) {
                ...
                if (fftCandidate !== null) {
                  const ratio = fftCandidate / smoothedFreq;
                  if (ratio >= CFIFTH_MIN_RATIO && ratio <= CFIFTH_MAX_RATIO) {
                    refinedCFifth = fftCandidate;
                  }
                }
              }
            ...
            const stableOctave = finalizeStableFrequency(octaveFramesRef.current, MIN_PARTIAL_FRAMES, 10);
            const stableCFifth = finalizeStableFrequency(cFifthFramesRef.current, MIN_PARTIAL_FRAMES, 22);
            ...
            smoothedOctaveRef.current =
              stableOctave === null
                ...
                : smoothedOctaveRef.current === null
                  ? stableOctave
                  : FREQ_SMOOTH_ALPHA * stableOctave + (1 - FREQ_SMOOTH_ALPHA) * smoothedOctaveRef.current;
            ...
            smoothedCFifthRef.current =
              stableCFifth === null
                ...
                : smoothedCFifthRef.current === null
                  ? stableCFifth
                  : FREQ_SMOOTH_ALPHA * stableCFifth + (1 - FREQ_SMOOTH_ALPHA) * smoothedCFifthRef.current;

            const emitNowMs = performance.now();
            if (emitNowMs - lastEmitMsRef.current < EMIT_INTERVAL_MS) {
              rafRef.current = requestAnimationFrame(tick);
              return;
            }
            lastEmitMsRef.current = emitNowMs;

            const refinementElapsed = Math.max(0, dt - lockEndMs);
            const refinementProgress = clamp(refinementElapsed / REFINE_WINDOW_MS, 0, 1);
            const refinementReady =
              refinementElapsed >= REFINEMENT_READY_MS ||
              (octaveFramesRef.current.length >= MIN_PARTIAL_FRAMES ||
                cFifthFramesRef.current.length >= MIN_PARTIAL_FRAMES) ||
            ...
            const nowMs = performance.now();
            const sustainGate = Math.max(
              SIGNAL_RMS_THRESHOLD * 0.7,
              noiseFloorRef.current * SUSTAIN_RMS_FACTOR + 0.00035,
            );

            const canHoldLastNote =
              smoothedFreqRef.current !== null &&
              lastGoodNominalFreqRef.current !== null &&
              lastGoodMidiRef.current !== null &&
              lastLockQualityRef.current >= SUSTAIN_MIN_LOCK_QUALITY &&
              nowMs - lastGoodDetectionMsRef.current <= HOLD_LAST_NOTE_MS &&
              rms >= sustainGate;

            if (canHoldLastNote) {
              rejectReasonRef.current = 'holding last locked note during sustain';
              silenceCountRef.current = 0;

              const nominalFreq = lastGoodNominalFreqRef.current!;
              const midiNote = lastGoodMidiRef.current!;
              const noteName = midiToFullName(midiNote);
              const rawCents = clamp(
                centsFromNominal(smoothedFreqRef.current!, nominalFreq),
                -60,
                60,
              );
              ...
              smoothedCentsRef.current =
                smoothedCentsRef.current === null
                  ? rawCents
                  : CENTS_SMOOTH_ALPHA * rawCents +
                    (1 - CENTS_SMOOTH_ALPHA) * smoothedCentsRef.current;
              ...
              setResult({
                ...
                noteName,
                cents: smoothedCentsRef.current,
                matchScore: lastGoodMatchScoreRef.current,
                lockQuality: lastLockQualityRef.current,
                refinementProgress: 1,
