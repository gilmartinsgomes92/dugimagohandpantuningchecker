...
const PARTIAL_IGNORE_AFTER_STRIKE_MS = 80;
const PARTIAL_LOCK_WINDOW_MS = 420;
const PARTIAL_REFINE_WINDOW_MS = 1800;

const MIN_OCTAVE_FRAMES = 2;
const MAX_OCTAVE_FRAMES = 24;
const MIN_CFIFTH_FRAMES = 2;
const MAX_CFIFTH_FRAMES = 24;

const OCTAVE_MAX_MAD_CENTS = 24;
const CFIFTH_MAX_MAD_CENTS = 30;

const PARTIAL_CLEAR_MISSES = 18;

const WIDE_OCTAVE_WINDOW_CENTS = 170;
const WIDE_CFIFTH_WINDOW_CENTS = 280;

const OCTAVE_MIN_RATIO = 1.62;
const OCTAVE_MAX_RATIO = 2.48;
const CFIFTH_MIN_RATIO = 2.22;
const CFIFTH_MAX_RATIO = 3.82;
...
function mergePartialCandidate(
  ...
  const fftErr = Math.abs(centsFromNominal(fftCandidate, targetFreq));
  const timeErr = Math.abs(centsFromNominal(timeCandidate, targetFreq));
          ...
          if (match !== null) {
            ...
            if (freq !== null) {
              ...
              if (
                ...
                let refinedOctave: number | null = findHarmonicFrequency(
                  freqBufRef.current,
                  octaveNominal,
                  sampleRate,
                  analyserRef.current.fftSize,
                ...
                if (refinedOctave !== null) {
                  const ratio = refinedOctave / smoothedFreq;
                  if (ratio < OCTAVE_MIN_RATIO || ratio > OCTAVE_MAX_RATIO) {
                    refinedOctave = null;
                  }
                }

                refinedOctave = mergePartialCandidate(
                  refinedOctave,
                ...
                if (refinedOctave !== null) {
                  ...
                  if (octaveMissesRef.current >= PARTIAL_CLEAR_MISSES) {
                    octaveMissesRef.current = PARTIAL_CLEAR_MISSES;
                  }
                }
                ...
                refinedCFifth = mergePartialCandidate(
                  refinedCFifth,
                ...
                if (refinedCFifth !== null) {
                  ...
                  if (cFifthMissesRef.current >= PARTIAL_CLEAR_MISSES) {
                    cFifthMissesRef.current = PARTIAL_CLEAR_MISSES;
                  }
                }
              }
