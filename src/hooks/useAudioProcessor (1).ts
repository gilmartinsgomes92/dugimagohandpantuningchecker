...
const PARTIAL_DISPLAY_DELAY_MS = 320;
const PARTIAL_TRACK_WINDOW_MS = 4200;
const PARTIAL_STABLE_WINDOW_MS = 1700;
const PARTIAL_HOLD_MS = 2800;
...
const OCTAVE_EXTREME_MAX_MAD_CENTS = 42;
const CFIFTH_EXTREME_MAX_MAD_CENTS = 48;
...
const EXTREME_LANE_MIN_DOMINANCE_DB = 5.0;
const EXTREME_LANE_MIN_TARGET_ERROR_CENTS = 110;
const PROVISIONAL_PARTIAL_RECENT_WINDOW_MS = 950;
const PROVISIONAL_PARTIAL_CLUSTER_CENTS = 55;
const PROVISIONAL_MIN_FRAMES = 2;
const PROVISIONAL_MIN_QUALITY = 0.5;
...
function finalizeStableFrequency(
  ...
  const initialRecentFrames = pruneFrames(frames, nowMs, stableWindowMs);
  if (initialRecentFrames.length < minFrames) return null;

  const initialCents = initialRecentFrames.map((f) => f.cents);
  const initialCentsMed = median(initialCents);
  const isExtreme = Math.abs(initialCentsMed) >= EXTREME_PARTIAL_THRESHOLD_CENTS;
  const recentFrames = isExtreme
    ? pruneFrames(frames, nowMs, Math.max(stableWindowMs, PARTIAL_HOLD_MS))
    : initialRecentFrames;

  const centsArr = recentFrames.map((f) => f.cents);
  const centsMed = median(centsArr);
  const effectiveMinFrames = isExtreme ? Math.max(minFrames, 5) : minFrames;
  const effectiveMadCents = isExtreme ? extremeMaxMadCents : maxMadCents;

  if (recentFrames.length < effectiveMinFrames) return null;

  const extremeFrames = recentFrames.filter((f) => f.lane === 'extreme').length;
  if (isExtreme && extremeFrames < Math.max(2, Math.ceil(effectiveMinFrames * 0.35))) return null;

  const centsMad = mad(centsArr, centsMed);
  if (centsMad > effectiveMadCents) return null;

  const selected = recentFrames
    .filter((f) => Math.abs(f.cents - centsMed) <= Math.max(effectiveMadCents * 1.8, 22))
    .sort((a, b) => (b.quality - a.quality) || (b.ts - a.ts))
    .slice(0, Math.max(effectiveMinFrames, Math.ceil(recentFrames.length * 0.7)))
    .map((f) => f.freq);
...
function finalizeProvisionalPartialFrequency(
  frames: WindowFrame[],
  harmonicType: 'octave' | 'compoundFifth',
  nowMs: number,
): number | null {
  const recentFrames = pruneFrames(frames, nowMs, PROVISIONAL_PARTIAL_RECENT_WINDOW_MS)
    .filter((f) => Math.abs(f.cents) >= EXTREME_PARTIAL_THRESHOLD_CENTS || f.lane === 'extreme')
    .sort((a, b) => (b.quality - a.quality) || (b.ts - a.ts));

  if (recentFrames.length < PROVISIONAL_MIN_FRAMES) return null;

  const topFrames = recentFrames.slice(0, Math.min(4, recentFrames.length));
  const qualityMean = topFrames.reduce((sum, frame) => sum + frame.quality, 0) / topFrames.length;
  if (qualityMean < PROVISIONAL_MIN_QUALITY) return null;

  const centsArr = topFrames.map((f) => f.cents);
  const centsMed = median(centsArr);
  const centsMad = mad(centsArr, centsMed);
  const maxClusterCents = harmonicType === 'octave'
    ? PROVISIONAL_PARTIAL_CLUSTER_CENTS
    : PROVISIONAL_PARTIAL_CLUSTER_CENTS + 8;
  if (centsMad > maxClusterCents) return null;

  const clustered = topFrames
    .filter((f) => Math.abs(f.cents - centsMed) <= Math.max(maxClusterCents, 28))
    .map((f) => f.freq);

  if (clustered.length < PROVISIONAL_MIN_FRAMES) return null;
          ...
          if (match !== null) {
            ...
            if (freq !== null) {
              ...
              const stableOctave = canDisplayPartials
                ...
                : null;
              const provisionalOctave =
                stableOctave === null && canDisplayPartials
                  ? finalizeProvisionalPartialFrequency(
                      octaveFramesRef.current,
                      'octave',
                      nowMs,
                    )
              ...
              const stableCFifth = canDisplayPartials
                ...
                : null;
              const provisionalCFifth =
                stableCFifth === null && canDisplayPartials
                  ? finalizeProvisionalPartialFrequency(
                      cFifthFramesRef.current,
                      'compoundFifth',
                      nowMs,
              ...
              } else if (provisionalOctave !== null) {
                octaveLastStableMsRef.current = nowMs;
                smoothedOctaveRef.current =
                  smoothedOctaveRef.current === null
                    ? provisionalOctave
                    : 0.12 * provisionalOctave + 0.88 * smoothedOctaveRef.current;
              } else if (
                smoothedOctaveRef.current !== null &&
              ...
              } else if (provisionalCFifth !== null) {
                cFifthLastStableMsRef.current = nowMs;
                smoothedCFifthRef.current =
                  smoothedCFifthRef.current === null
                    ? provisionalCFifth
                    : 0.12 * provisionalCFifth + 0.88 * smoothedCFifthRef.current;
              } else if (
                smoothedCFifthRef.current !== null &&
