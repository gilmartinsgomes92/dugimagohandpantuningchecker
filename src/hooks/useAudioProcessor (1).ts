...
function finalizeProvisionalFrequency(
  ...
  if (qualityMed < 0.43) return null;
...
export const useAudioProcessor = () => {
  ...
  // Silence grace
  const silenceDeadlineMsRef = useRef(0);
  ...
  const shouldClearForSilence = useCallback((nowMs: number) => {
    if (silenceDeadlineMsRef.current === 0) {
      silenceDeadlineMsRef.current = nowMs + SILENCE_GRACE_MS;
      return false;
    }
    return nowMs >= silenceDeadlineMsRef.current;
  }, []);

  const resetState = useCallback(() => {
    silenceDeadlineMsRef.current = 0;
...
if (rms >= dynamicGate) {
          ...
          if (match !== null) {
            ...
            if (freq !== null) {
              ...
              if (shouldClearForSilence(nowMs)) {
                resetState();
                setResult({
                  frequency: null,
                  octaveFrequency: null,
  ...
  }, [resetState, shouldClearForSilence]);
