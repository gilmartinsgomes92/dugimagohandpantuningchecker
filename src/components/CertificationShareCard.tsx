...
export default function CertificationShareCard({
  ...
  brandName = 'Dugimago',
  appUrl = 'tuner.dugimago.com/verify',
  verificationId = 'Pending',
}: CertificationShareCardProps) {
  const measuredAggregates = orderCertificationAggregates(aggregates.filter((aggregate) => aggregate.strikeCount > 0));
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 20 }}>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ color: '#94a3b8', fontSize: 14, marginTop: 6 }}>
              Verify this report at {appUrl} using the Verification ID shown above.
            </div>
          </div>
        </div>
      </div>
