...
const REGISTRATION_COOLDOWN_MS = 1200;
const MIN_STABLE_FRAMES_TO_REGISTER = 3;
const MIN_COLLECTION_LOCK = 0.48;
...
useEffect(() => {
  ...
  const registerNote = useCallback(() => {
    ...
    // Use only actually measured partials.
    // Never synthesize octave / compound fifth from the fundamental because that
    // fabricates trustworthy-looking data when the partial was not really captured.
    const OCTAVE_MAX_TARGET_CENTS = 180;
    const CFIFTH_MAX_TARGET_CENTS = 260;
    ...
    const octaveFreq = useMeasuredOctave ? rawOctave : undefined;
    const octaveCents = octaveFreq !== undefined ? 1200 * Math.log2(octaveFreq / targetOctaveFreq) : undefined;
    ...
    const compoundFifthFreq = useMeasuredCFifth ? rawCFifth : undefined;
    const compoundFifthCents =
      compoundFifthFreq !== undefined ? 1200 * Math.log2(compoundFifthFreq / targetCompoundFifthFreq) : undefined;
  ...
  const hasEnoughStableFrames = stableFrequencies.current.length >= MIN_STABLE_FRAMES_TO_REGISTER;
  const shouldRegister =
    hasEnoughStableFrames &&
    (stabilityPct >= 90 || isLockedForRegister || result.refinementReady);
  ...
  useEffect(() => {
    ...
    if (lockQ >= MIN_COLLECTION_LOCK) {
      stableFrequencies.current.push(result.frequency);
      if (stableFrequencies.current.length > 8) stableFrequencies.current.shift();

      if (result.octaveFrequency !== null) {
        stableOctaveFreqs.current.push(result.octaveFrequency);
        if (stableOctaveFreqs.current.length > 8) stableOctaveFreqs.current.shift();
      }

      if (result.compoundFifthFrequency !== null) {
        stableCFifthFreqs.current.push(result.compoundFifthFrequency);
        if (stableCFifthFreqs.current.length > 8) stableCFifthFreqs.current.shift();
      }
    }
