...
const LOCK_THRESHOLD = IS_IOS ? 0.7 : 0.75;

// Minimum number of sustain-phase frequency samples required before registration.
// With the new strike-window lock (median+MAD) in the audio hook, 1 sample is enough.
...
const QuickTuningPage: React.FC = () => {
  ...
  // Stability ring shows the audio lock quality (scaled so a "register-level" lock reaches 100%)
  const lockQuality = result.lockQuality ?? 0;
  const stabilityPct = Math.round(Math.min(1, lockQuality / LOCK_THRESHOLD) * 100);
