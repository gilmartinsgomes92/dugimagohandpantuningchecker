...
function fullNameToMidi(fullName: string): number | null {
  const match = /^([A-G](?:#|b)?)(-?\d+)$/.exec(fullName.trim());
...
const QuickTuningPage: React.FC = () => {
  ...
  const registerNote = useCallback(() => {
    ...
    if (detectedFreq === null || noteName === null) return;

    const midiNote = fullNameToMidi(noteName) ?? frequencyToNote(detectedFreq).midiNote;
    const cents = 1200 * Math.log2(detectedFreq / midiToFrequency(midiNote));
    ...
    // Keep independently measured partials whenever they exist.
    // In very detuned handpans the fundamental may snap to the nearest semitone,
    // while octave / compound fifth can still sit much closer to the original
    // note family. Only synthesize 2x/3x values when measurement is missing or
    // clearly non-physical.
    const MAX_REASONABLE_OCTAVE_CENTS = 240;
    const MAX_REASONABLE_CFIFTH_CENTS = 240;
    ...
    const octaveFreq =
      rawOctave !== null &&
      rawOctaveCents !== null &&
      Number.isFinite(rawOctaveCents) &&
      Math.abs(rawOctaveCents) <= MAX_REASONABLE_OCTAVE_CENTS
        ? rawOctave
        : detectedFreq * 2;
    ...
    const compoundFifthFreq =
      rawCFifth !== null &&
      rawCFifthCents !== null &&
      Number.isFinite(rawCFifthCents) &&
      Math.abs(rawCFifthCents) <= MAX_REASONABLE_CFIFTH_CENTS
        ? rawCFifth
        : detectedFreq * 3;
