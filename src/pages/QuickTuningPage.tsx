...
useEffect(() => {
  ...
  const registerNote = useCallback(() => {
    ...
    // Partials must be truly measured.
    // Never fabricate 2× / 3× values when the app did not really capture them.
    const targetOctaveFreq = midiToFrequency(midiNote + 12);
    const targetCompoundFifthFreq = midiToFrequency(midiNote + 19);

    const octaveFreq = trimmedMean(stableOctaveFreqs.current);
    const octaveCents = octaveFreq !== null ? 1200 * Math.log2(octaveFreq / targetOctaveFreq) : null;

    const compoundFifthFreq = trimmedMean(stableCFifthFreqs.current);
  ...
  const refinementReady = result.refinementReady || (result.refinementProgress ?? 0) >= 0.7;
  const shouldRegister = (stabilityPct >= 98 || isLockedForRegister) && refinementReady;
