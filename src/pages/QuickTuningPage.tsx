...
const QuickTuningPage: React.FC = () => {
  ...
  const registerNote = useCallback(() => {
    ...
    if (detectedFreq === null || result.noteName === null) return;

    // Anchor registration to the hook's chosen note identity. This preserves the
    // nearest-semitone snap for extreme detune cases (e.g. a very sharp D3 being
    // identified as Eb3) instead of re-deriving note identity from the averaged
    // measured frequency here.
    const noteData = frequencyToNote(detectedFreq);
    const cents = noteData.cents;
    const noteName = result.noteName;
    const noteNameMatch = /^([A-G]#?)(-?\d+)$/.exec(noteName);
    if (!noteNameMatch) return;
    ...
    const targetMidi = (Number(noteNameMatch[2]) + 1) * 12 + noteIndexMap[noteNameMatch[1]];
    ...
    // Important: do NOT validate them against detectedFreq * 2 / *3. In extreme
    // detune cases the snapped fundamental may belong to the adjacent semitone,
    // while the physical partials are still closer to the original note family.
    // If we compare to detectedFreq ratios here, we throw away the real measured
    // partials and replace them with synthetic values, causing the false
    // "+6 / +6 / +6" style result.
    const MAX_REASONABLE_PARTIAL_CENTS = 260;

    const targetOctaveFreq = midiToFrequency(targetMidi + 12);
    ...
    const octaveFreq =
      rawOctave !== null &&
      rawOctaveCents !== null &&
      Math.abs(rawOctaveCents) <= MAX_REASONABLE_PARTIAL_CENTS
        ? rawOctave
        : rawOctave !== null
          ? rawOctave
          : detectedFreq * 2;
    ...
    const targetCompoundFifthFreq = midiToFrequency(targetMidi + 19);
    ...
    const compoundFifthFreq =
      rawCFifth !== null &&
      rawCFifthCents !== null &&
      Math.abs(rawCFifthCents) <= MAX_REASONABLE_PARTIAL_CENTS
        ? rawCFifth
        : rawCFifth !== null
          ? rawCFifth
          : detectedFreq * 3;
