...
const QuickTuningPage: React.FC = () => {
  ...
  const stableOctaveDiagnosticFreqs = useRef<number[]>([]);
  const stableCFifthFreqs = useRef<number[]>([]);
  const stableCFifthDiagnosticFreqs = useRef<number[]>([]);
  ...
  const resetStabilityState = useCallback(() => {
    ...
    stableOctaveDiagnosticFreqs.current = [];
    stableCFifthFreqs.current = [];
    stableCFifthDiagnosticFreqs.current = [];
  }, []);
...
useEffect(() => {
  ...
  const registerNote = useCallback(() => {
    ...
    const robustPartialMean = (freqs: number[]): number | null => {
      if (freqs.length === 0) return null;

      const sorted = [...freqs].sort((a, b) => a - b);

      if (sorted.length <= 2) {
        return sorted.reduce((sum, f) => sum + f, 0) / sorted.length;
      }
      ...
      if (sorted.length <= 5) {
        ...
        return trimmed.reduce((sum, f) => sum + f, 0) / trimmed.length;
      }
    ...
    const DIAGNOSTIC_EXTREME_MIN_CENTS = 100;

    const targetOctaveFreq = midiToFrequency(midiNote + 12);
    const targetCompoundFifthFreq = midiToFrequency(midiNote + 19);

    const rawOctave = robustPartialMean(stableOctaveFreqs.current);
    const rawOctaveCents =
      rawOctave !== null ? 1200 * Math.log2(rawOctave / targetOctaveFreq) : null;
    const rawDiagnosticOctave = robustPartialMean(stableOctaveDiagnosticFreqs.current);
    const rawDiagnosticOctaveCents =
      rawDiagnosticOctave !== null ? 1200 * Math.log2(rawDiagnosticOctave / targetOctaveFreq) : null;
    const useMeasuredOctave =
      rawOctave !== null && rawOctaveCents !== null && Math.abs(rawOctaveCents) <= OCTAVE_MAX_TARGET_CENTS;
    const useDiagnosticExtremeOctave =
      !useMeasuredOctave &&
      rawDiagnosticOctave !== null &&
      rawDiagnosticOctaveCents !== null &&
      Math.abs(rawDiagnosticOctaveCents) >= DIAGNOSTIC_EXTREME_MIN_CENTS;
    const octaveSource: 'measured' | 'diagnostic-extreme' | 'estimated' =
      useMeasuredOctave
        ? 'measured'
        : useDiagnosticExtremeOctave
          ? 'diagnostic-extreme'
          : 'estimated';
    const octaveFreq =
      octaveSource === 'measured'
        ? rawOctave
        : octaveSource === 'diagnostic-extreme'
          ? rawDiagnosticOctave
          : detectedFreq * 2;
    const octaveCents =
      octaveFreq !== null ? 1200 * Math.log2(octaveFreq / targetOctaveFreq) : null;

    const rawCFifth = robustPartialMean(stableCFifthFreqs.current);
    const rawCFifthCents =
      rawCFifth !== null ? 1200 * Math.log2(rawCFifth / targetCompoundFifthFreq) : null;
    const rawDiagnosticCFifth = robustPartialMean(stableCFifthDiagnosticFreqs.current);
    const rawDiagnosticCFifthCents =
      rawDiagnosticCFifth !== null ? 1200 * Math.log2(rawDiagnosticCFifth / targetCompoundFifthFreq) : null;
    const useMeasuredCFifth =
      rawCFifth !== null && rawCFifthCents !== null && Math.abs(rawCFifthCents) <= CFIFTH_MAX_TARGET_CENTS;
    const useDiagnosticExtremeCFifth =
      !useMeasuredCFifth &&
      rawDiagnosticCFifth !== null &&
      rawDiagnosticCFifthCents !== null &&
      Math.abs(rawDiagnosticCFifthCents) >= DIAGNOSTIC_EXTREME_MIN_CENTS;
    const compoundFifthSource: 'measured' | 'diagnostic-extreme' | 'estimated' =
      useMeasuredCFifth
        ? 'measured'
        : useDiagnosticExtremeCFifth
          ? 'diagnostic-extreme'
          : 'estimated';
    const compoundFifthFreq =
      compoundFifthSource === 'measured'
        ? rawCFifth
        : compoundFifthSource === 'diagnostic-extreme'
          ? rawDiagnosticCFifth
          : detectedFreq * 3;
    const compoundFifthCents =
      compoundFifthFreq !== null ? 1200 * Math.log2(compoundFifthFreq / targetCompoundFifthFreq) : null;
    ...
    const payload: TuningResult = {
      ...
      status,
      compoundFifthFreq:
        compoundFifthSource === 'estimated' ? undefined : compoundFifthFreq ?? undefined,
      compoundFifthCents:
        compoundFifthSource === 'estimated' ? undefined : compoundFifthCents ?? undefined,
      compoundFifthSource,
      octaveFreq:
        octaveSource === 'estimated' ? undefined : octaveFreq ?? undefined,
      octaveCents:
        octaveSource === 'estimated' ? undefined : octaveCents ?? undefined,
      octaveSource,
    };
  ...
  useEffect(() => {
    ...
    if (lockQ >= 0.55) {
      ...
      if (result.octaveDiagnosticFrequency !== null) {
        stableOctaveDiagnosticFreqs.current.push(result.octaveDiagnosticFrequency);
      }
      if (result.compoundFifthFrequency !== null) {
        stableCFifthFreqs.current.push(result.compoundFifthFrequency);
      }
      if (result.compoundFifthDiagnosticFrequency !== null) {
        stableCFifthDiagnosticFreqs.current.push(result.compoundFifthDiagnosticFrequency);
      }
    }
