...
function hasEnoughPartialEvidence(octaveFrames: number[], cFifthFrames: number[]): boolean {
  return (
    octaveFrames.length >= PARTIAL_MIN_SAMPLES ||
    cFifthFrames.length >= PARTIAL_MIN_SAMPLES
  );
}
...
const QuickTuningPage: React.FC = () => {
  ...
  const [instructionText, setInstructionText] = useState('Hold the note ringing — it will be auto-registered');
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    } else if (sameNoteSince.current === 0) {
      sameNoteSince.current = now;
    }
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (justRegistered.current) {
      ...
      if (collectingNoteName.current !== result.noteName) {
        sameNoteSince.current = performance.now();
      }
      collectingNoteName.current = result.noteName;
    }
...
if (lockQ >= 0.55) {
  ...
  if (noteCaptureStartedAt.current === 0) {
    noteCaptureStartedAt.current = performance.now();
  }
}
...
if (result.compoundFifthFrequency !== null) {
  stableCFifthFreqs.current.push(result.compoundFifthFrequency);
    ...
    const captureAgeMs = noteCaptureStartedAt.current > 0
      ? performance.now() - noteCaptureStartedAt.current
      : 0;
    ...
    if (shouldRegister && !justRegistered.current) {
      ...
      const sameNoteAgeMs = sameNoteSince.current > 0 ? now - sameNoteSince.current : 0;
      const enoughPartials = hasEnoughPartialEvidence(
        stableOctaveFreqs.current,
        stableCFifthFreqs.current,
      );
  ...
  const liveFundamentalCents =
    result.frequency !== null && liveFundamentalTarget !== null
      ? 1200 * Math.log2(result.frequency / liveFundamentalTarget)
      : null;
  const liveOctaveCents =
    result.octaveFrequency !== null && liveOctaveTarget !== null
  ...
  const liveFundamentalColor =
    liveFundamentalCents !== null ? centsToColor(liveFundamentalCents) : '#555';
  const liveOctaveColor = liveOctaveCents !== null ? centsToColor(liveOctaveCents) : '#555';
  const liveCompoundFifthColor =
    liveCompoundFifthCents !== null ? centsToColor(liveCompoundFifthCents) : '#555';
            ...
            {result.frequency !== null ? (
              <>
                ...
                >
                  ...
                  >
                    <div className="reading-label" style={{ marginBottom: '0.35rem' }}>
                      Fundamental
                    </div>
                    <div
                      className="reading-value"
                      style={{ color: liveFundamentalColor, fontSize: '1.05rem', fontWeight: 700 }}
                    >
                      {liveFundamentalCents !== null ? formatCents(liveFundamentalCents) : '—'}
                    </div>
                    <div style={{ fontSize: '0.82rem', opacity: 0.75, marginTop: '0.25rem' }}>
                      {result.frequency !== null ? `${result.frequency.toFixed(2)} Hz` : 'Listening…'}
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div
                      className="reading-value"
                      style={{ color: liveOctaveColor, fontSize: '1.05rem', fontWeight: 700 }}
                    >
                      {liveOctaveCents !== null ? formatCents(liveOctaveCents) : '—'}
                    </div>
                    <div style={{ fontSize: '0.82rem', opacity: 0.75, marginTop: '0.25rem' }}>
                      {result.octaveFrequency !== null
                        ? `${result.octaveFrequency.toFixed(2)} Hz`
                        : isListening
                          ? 'Listening…'
                          : '—'}
                    </div>
                  ...
                  >
                    ...
                    <div
                      className="reading-value"
                      style={{ color: liveCompoundFifthColor, fontSize: '1.05rem', fontWeight: 700 }}
                    >
                      {liveCompoundFifthCents !== null ? formatCents(liveCompoundFifthCents) : '—'}
                    </div>
                    <div style={{ fontSize: '0.82rem', opacity: 0.75, marginTop: '0.25rem' }}>
                      {result.compoundFifthFrequency !== null
                        ? `${result.compoundFifthFrequency.toFixed(2)} Hz`
                        : isListening
                          ? 'Listening…'
                          : '—'}
                    </div>
