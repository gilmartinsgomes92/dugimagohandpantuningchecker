...
// When the final note is registered, keep listening a bit longer so late
// octave / compound-fifth frames can still land before auto-navigation.
const FINAL_NOTE_RESULTS_DELAY_MS = REGISTRATION_COOLDOWN_MS + 250;
...
const QuickTuningPage: React.FC = () => {
  ...
  const finalResultsTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
...
useEffect(() => {
  ...
  // Important: do not jump immediately on the last note, because we still want
  // to collect late octave / compound-fifth frames for a short tail window.
  useEffect(() => {
    if (finalResultsTimerRef.current) {
      clearTimeout(finalResultsTimerRef.current);
      finalResultsTimerRef.current = null;
    ...
    if (notesCount > 0 && registeredCount >= notesCount) {
      finalResultsTimerRef.current = setTimeout(() => {
        stopListening();
        navigate('/results');
      }, FINAL_NOTE_RESULTS_DELAY_MS);
    }
