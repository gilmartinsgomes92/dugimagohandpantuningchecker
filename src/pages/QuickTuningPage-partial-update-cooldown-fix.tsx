...
const QuickTuningPage: React.FC = () => {
  ...
  const lastRegisteredNoteName = useRef<string | null>(null);
...
useEffect(() => {
  ...
  const registerNote = useCallback(() => {
    ...
    lastRegisteredNoteName.current = noteName;
  ...
  useEffect(() => {
    ...
    const isCoolingDownSameNote = justRegistered.current && result.noteName === lastRegisteredNoteName.current;
    ...
    if (!isCoolingDownSameNote && !justRegistered.current && shouldRegister) {
      registerNote();
    }
